...
func (s ParcelStore) Add(p Parcel) (int, error) {
	...
	if err != nil {
		log.Printf("error adding parcel: %s", err)
		return 0, err
	}
	// верните идентификатор последней добавленной записи
	lastInsertId, err := res.LastInsertId()
	if err != nil {
		log.Printf("error getting last inserted parcel's ID: %s", err)
		return 0, err
	}
...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	...
	if err != nil {
		log.Printf("error quering rows: %s", err)
		return nil, err
	}
	defer rows.Close()
...
func (s ParcelStore) SetStatus(number int, status string) error {
	...
	if err != nil {
		log.Printf("error executing UPDATE command with parcel.SetStatus: %s", err)
		return err
	}
	return nil
}
...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	if err != nil {
		log.Printf("error executing UPDATE command with parcel.SetAddress func: %s", err)
		return err
	}
	return nil
}
...
func (s ParcelStore) Delete(number int) error {
	...
	_, err := s.db.Exec(
		...
		sql.Named("status", ParcelStatusRegistered))
	if err != nil {
		log.Printf("error executing DELETE command with parcel.Delete: %s", err)
		return err
	}
	return nil
}
